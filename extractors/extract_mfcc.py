...
def mfcc(filepath: str, win_length: int = 25, hop_length: int  = 10,n_fft: int = 512,n_mels:int = 40,fmin: float = 0.0, fmax: float = None):
    '''
    ...
    mel_basis = librosa.filters.mel(
        file_sr, n_fft, n_mels = n_mels, fmin=fmin, fmax=fmax)
...
def extract_mfcc(scp_file: str, output_ark: str, ceps: int = 13,win_length: int = 25, hop_length: int = 10, n_fft: int = 512,
    ...
    with open(output_ark, 'wb') as wp:
        for line in tqdm(file_key_path, ncols=100):
            feat = mfcc(line[1], win_length=win_length, hop_length=hop_length,n_fft=n_fft,
