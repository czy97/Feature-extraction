...
def extract_imf(scp_file: str, output_ark: str, sr: int = None, win_length: int = 25, hop_length: int = 10, fmin: int = 0.0, fmax: int = None, n_mels: int = 64, n_fft=512):
    ...
    with open(scp_file, 'r') as rp, open(output_ark, 'wb') as wp:
        for line in tqdm(rp, total=get_num_lines(scp_file)):
            ...
            inv_lms = np.log(np.dot(inverse_mel_basis, S) + 1e-12).transpose()
