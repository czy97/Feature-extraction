...
def extractraw(scp_file: str, output_ark: str, sr: int = None, win_length: int = 25, hop_length: int = 10, n_fft: int = 512):
    ...
    with open(scp_file, 'r') as rp, open(output_ark, 'wb') as wp:
        for line in tqdm(rp, total=get_num_lines(scp_file)):
            ...
            feature = np.log(np.abs(np.concatenate((S, H, P)))**2 + 1e-12).transpose()
